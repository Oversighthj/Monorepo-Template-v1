...
* **Frontends – Flutter Apps** (`admin_app`, `guest_app`, `cleaner_app`): Three separate Flutter projects provide UIs for different roles. Each app uses the **generated Dart API client** from `template_api` to communicate with the backend. They also depend on `packages/ui_kit` so shared widgets maintain a consistent look across apps. The projects are organized by feature modules and leverage Riverpod for state management, supporting web, mobile, and desktop builds.
