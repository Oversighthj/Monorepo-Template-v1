...
public class TaskController implements TaskApi {
    ...
    @GetMapping("/tasks/{id}")
    public ResponseEntity<TaskDTO> tasksIdGet(@PathVariable Long id) {
    ...
    public ResponseEntity<TaskDTO> tasksIdPut(
        ...
        UserEntity cleaner = userRepository
                .findById(dto.getCleanerId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Cleaner not found"));
        ...
        existing.setStatus(TaskStatus.valueOf(dto.getStatus().name()));
        existing.setDue(dto.getDue().toLocalDateTime());
